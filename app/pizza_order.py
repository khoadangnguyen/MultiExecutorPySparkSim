...
pizzas_df = spark.read.csv(
    "./MultiExecutorPySparkSim/data/pizzas.csv",
    header=True,
    inferSchema=True
)
...
pizza_types_df = spark.read.csv(
    "./MultiExecutorPySparkSim/data/pizza_types.csv",
    header=True,
    inferSchema=True
)
...
pizzas_pizza_types_df = pizzas_df.join(pizza_types_df,
                                       pizzas_df["pizza_type_id"] == pizza_types_df["pizza_type_id"],
                                       "inner")
highest_price_pizza_df = pizzas_pizza_types_df.filter(col("price") == highest_price)
highest_price_pizza_rows = highest_price_pizza_df.collect()
for row in highest_price_pizza_rows:
    print(f"\tPizza name: {row['name']} size: {row['size']}")
...
ordered_quantity_by_category.rdd.foreach(
    lambda row: print(f"\tCategory: {row['category']}, Total ordered quantity = {row['total_ordered_quantity']}"))
